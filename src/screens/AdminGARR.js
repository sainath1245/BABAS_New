...
const AdminGARR = ({navigation}) => {
  ...
  const checkValidation = () => {
    ...
    if (dobToShow === 'Select Date') {
      Alert.alert('Alert!', 'Please select report start date.');
    } else if (selectedValue === '') {
      Alert.alert('Alert!', 'Please select specific time.');
    } else {
      generateReport();
    }
  };
  ...
  const generateReport = async () => {
    ...
    if (selectedValue === '8') {
      selectedDateWithTime = startDateToSend_1 + 'T08:00';
    } else if (selectedValue === '10') {
      selectedDateWithTime = startDateToSend_1 + 'T10:00';
    } else if (selectedValue === '15') {
      selectedDateWithTime = startDateToSend_1 + 'T15:00';
    } else if (selectedValue === '20') {
      selectedDateWithTime = startDateToSend_1 + 'T20:00';
    }

    console.log('selectedValue', selectedDateWithTime);
    ...
    const requestOptions = {
      ...
      body: JSON.stringify({
        ...
        //"startDate":"2024-05-27T15:00" for dev server
      }),
    };
    console.log('--requestOptions.body--:' + requestOptions.body);
    await fetch(BASE_URL + 'Admin/ExportAttendanceInfo', requestOptions)
      .then(response => {
        if (response.ok) {
      ...
      .then(data => {
        ...
        if (json.responseCode == 200) {
          Alert.alert('Alert!', 'Report exported successfully.', [
            {
              text: 'OK',
              onPress: () => {
            ...
            <View style={historyDetailPageStyles.btn_container}>
              ...
              <TouchableOpacity
                ...
                {loading ? (
                  ...
                  <Text style={loginPageStyles.btn_text}>Generate Report</Text>
                )}
              </TouchableOpacity>
            </View>
          </View>
