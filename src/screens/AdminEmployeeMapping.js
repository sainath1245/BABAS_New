...
const AdminEmployeeMapping = ({navigation}) => {
  ...
  // This function is to get All Employees from server
  const getEmployees = async (token) => {
    ...
    await fetch(BASE_URL + 'Admin/GetEmployeeList', requestOptions)
      .then(response => {
      ...
      .then(data => {
        ...
        if (json.responseCode == 200) {
          ...
          Alert.alert('Alert!', data.responseMessage);
        }
      })
      .catch(error => {
      ...
      <View style={historyPageStyles.list_main_container}>
        <View style={adminSuperVisorMapping.list_second_container}>
          <View style={{padding: 5}}>
            <Text numberOfLines={1} style={adminSuperVisorMapping.title_text}>
              {item.employeeName}
            </Text>
            ...
            <TouchableOpacity
              ...
              <View
                style={{
                  ...
                  backgroundColor: '#E63627',
                  height: 120,
                }}>
                <Text style={adminSuperVisorMapping.assign_to_text}>
                  Assign To
                </Text>
                <View style={historyPageStyles.circle_1}>
                  <Image
              ...
              setDataArray_1(
                dataArray.filter(function (el) {
                  if (isNaN(value)) {
                    ...
                    return el.employeeName
                      .toLowerCase()
                      .includes(value.toLowerCase());
                  } else {
                    console.log('==is value string== ' + value);
                    return el.employeeID == value;
                  }
                }),
              );
            }}
...
export default AdminEmployeeMapping;
